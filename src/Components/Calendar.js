...
const Calendar = ({ selectedArtist })  => {
  ...
  [
    ...
    { name: 'عروسه', id: '1' },
    { name: 'مكياج سهره', id: '2' },
    { name: 'تسريحه شعر', id: '3' },
    { name: 'تركيب رموش', id: '4' },

    { name: 'عكف رموش', id: '5' },
    { name: 'رفع حواجب', id: '6' },
    { name: 'تركيب اظافر اكريلك', id: '7' },
    { name: 'تركيب اظافر جل', id: '8' },
    { name: 'مساج', id: '9' },
    { name: 'تنظيف بشره', id: '10' },
    { name: 'اكستنشن دائم للشعر', id: '11' },
...
async function handleReservation (e) {
  ...
  const requestData = {
    ...
    'event': Selectedreservationtype, // Use the mapped event number
    ...
    axios
      ...
      .then((response) => {
        ...
        const formattedFreeSlots = freeSlots.map((slot) =>
          slot.toLocaleTimeString('en-US', { hour12: false, hour: 'numeric', minute: '2-digit' })
        );
...
const generateCalendarDays = () => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    const dayClassName = `p-2 cursor-pointer text-center ${
      isSelected ? 'bg-[#F5C890] text-white' : isHoliday ? 'bg-[#F5C89073] text-white' : 'bg-[#D9D9D9]'
    }`;
...
//   console.log(filteredTimes[filteredTimes.length - 1])
    ...
    while (nextIndex < AvailableTime.length) {
      ...
      console.log()
      //const formattedNextTime = moment(nextTime, 'h:mm A').format('h:mm A');
...
return (
  ...
  <button className='bg-[#F9C688] w-32 p-1 text-center text-white shadow-inner shadow-[#FFFFFF]' onClick={handleReservation}>حفظ</button>
  <h2 className='font-bold text-xl'>{selectedArtist?selectedArtist.name:''}</h2>
  </div>
...
<div className='w-3 h-3 bg-[#F5C890] items-center mt-2 mr-5 ml-2'></div>
  ...
  <h2 className='ml-1 text-xs whitespace-nowrap mt-2'>:بدء الموعد</h2>
  </div>
  <div className='flex'>
    <div className='w-28'>
  <Start paasedarr={getFilteredEndTimes(selectedStartTime)} onSelectTime={handleendTimeSelection} />
  </div>
  <h2 className='ml-1 text-xs whitespace-nowrap mt-2'>:نهاية الموعد</h2>
  </div>
  </div>
  <div className='flex justify-end mt-10 '>
    <Reservation  clients={clients} onSelectClient={handleClientSelection}/>
    <h2 className='text-right mb-5 text-xs whitespace-nowrap mt-2'>: حجز بإسم<span className='font-bold'>.</span></h2>
  </div>
   <div className='flex justify-end mt-10'>
    <div className='w-[82%]'>
   <DropDownMultiSelect paasedarr={reservationTypeMappings}  onSelectTime={handlereservationtype}/>
   </div>
    <h2 className='text-right mb-5  text-xs ml-3 mt-2'>: المناسبة <span className='font-bold'>.</span></h2>
  </div>
