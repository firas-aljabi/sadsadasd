...
const Reservation = ({clients, onSelectClient }) => {
  ...
  return (
    ...
    <div className="w-full font-medium flex justify-end">
      ...
      >
        <img src={chevron} size={20} color="" className={`absolute left-3 w-4 h-4 ${open && "rotate-180 "}`} /> 
         <input
            type="text"
            value= {
          inputValue}
            ...
            className="placeholder:text-[#D4821F] placeholder:font-normal placeholder:pl-2 p-3 w-28 text-center text-[#D4821F] text-[12px] border outline-none border-black bg-[#F7F7F8]"
          />
      </div>
      <ul
