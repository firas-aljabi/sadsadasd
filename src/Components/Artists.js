    ...
    return artistsData.map((artist, index) => (
      ...
      >
        ...
        <td className={` ${selectedRow === index ? 'bg-[#F9C688]' : 'bg-[#E5E5E5]'} flex flex-col justify-center sm:flex-row sm:justify-between p-3`}>
          <p className='text-center mt-3 lg:-mr-16 sm:text-right sm:w-24'>{artist.name}</p>
          <img src={artist.image} className='w-14 h-14  sm:mx-0 rounded-full' alt={artist.name} />
        </td>
      </tr> 
    ));
  };
