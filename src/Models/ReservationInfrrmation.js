...
const getReservationTypes = (amountTypeArray) => {
  if (!amountTypeArray || amountTypeArray.length === 0) {
    return '';
  }

  const numericAmountTypes = amountTypeArray.map((amountType) => parseInt(amountType));

  const matchingReservationTypes = Object.entries(reservationTypeMappings)
    .filter(([reservationType, numericValue]) => numericAmountTypes.includes(numericValue))
    .map(([reservationType]) => reservationType);

  return matchingReservationTypes.join(', '); // Join array elements with comma and space
};
            ...
            <SmallDropDown paasedarr={amountopPayment} name={" العربون"} onSelectTime={handleselectedamount}/>
              ...
              <div className='flex justify-between gap-2'>
               <div className=' bg-[#F7F7F8] border mb-3 w-[100%] p-0.5'>
                ...
                <h4 className='text-right text-[10px] opacity-100 mr-2'>{getReservationTypes(reservationData.amount_type)}</h4>
            </div>
              </div>
              <div className='flex justify-between gap-2'>
               <div className=' bg-[#F7F7F8] border mb-3 w-[100%] p-0.5'>
